...
import { RejectAccessRequestSchema, MessageResponseSchema } from '@/lib/schemas';
import { z } from 'zod';
...
export async function POST(
  ...
  try {
    ...
    const validatedData = RejectAccessRequestSchema.parse(body);
    ...
    const { error: updateError } = await supabase
      ...
      .update({
        status: 'rejected',
        rejected_reason: validatedData.rejected_reason,
    ...
    if (updateError) {
      ...
      return NextResponse.json(
        { error: 'Failed to reject access request', success: false },
        { status: 500 }
      );
    }
    ...
    console.error('API Error in POST /api/access-requests/[id]/reject:', error);
